...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'polls' | 'categories'>('polls');
      ...
      <div className="max-w-4xl mx-auto p-6">
        <div className="grid grid-cols-3 gap-4 mb-6">
          {[
            ...
            { label: 'Categorias', value: categories.length, emoji: '🏷️', color: '#FF4E8C' },
          ].map(s => (
          ...
          <>
            ...
            <div className="space-y-3">
              {filtered.map((poll) => {
                ...
                return (
                  <div key={poll.id} className="rounded-2xl p-5 border transition-all" style={{ background: poll.pinned ? 'rgba(247,183,49,0.05)' : 'rgba(255,255,255,0.03)', borderColor: poll.pinned ? '#F7B73133' : 'rgba(255,255,255,0.07)' }}>
          ...
          <div>
            ...
            <div className="flex gap-3 mb-6">
              <input
                ...
                placeholder="Nova categoria (ex: política, moda, finanças...)"
                value={newCat}
                ...
                return (
                  <div key={cat} className="flex items-center justify-between p-4 rounded-xl border" style={{ background: 'rgba(255,255,255,0.03)', borderColor: 'rgba(255,255,255,0.07)' }}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {!isDefault && (
                        <button onClick={() => handleDeleteCategory(cat)} className="w-7 h-7 rounded-lg flex items-center justify-center text-xs" style={{ background: 'rgba(255,82,82,0.1)', color: '#FF5252' }}>✕</button>
                      )}
                    </div>
                  </div>
                );
